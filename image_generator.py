...
def generer_en_tete(draw, img, width, padding, y):
    ...
    if logo and logo_width > 0:
        ...
        draw.text((soustitre_x, y + logo.height + mm_to_px(2)), "Vente en gros et détail", font=font_soustitre, fill='#000000')
    ...
    contact_y = logo_bottom + mm_to_px(8)
...
def generer_infos_client_rectangle(draw, width, client_nom, client_info, y_position_logo_top):
    ...
    # Stat - pas d'espacement supplémentaire
...
def generer_tableau_bon_livraison(draw, width, y_start, produits, num_facture):
    ...
    for i in range(row_count):
        ...
        row_fill = '#ffffff' if i % 2 == 0 else '#f5f5f5'
        ...
        if i < len(produits):
            ...
            draw.text((x_offset + mm_to_px(5), text_y), designation, font=font_normal, fill='#333333')
            x_offset += col_widths['designation']
...
def generer_bas_facture(draw, width, y_start, total, avance, reste,
    ...
    """Génère le bas de facture avec montants alignés à droite"""
...
def generer_facture_proforma(client_nom, client_info, produits,
    ...
    # Tableau
    tableau_y = generer_tableau_bon_livraison(draw, width, date_y, produits, commande_id)

    # Bas de page avec séparation visuelle claire
