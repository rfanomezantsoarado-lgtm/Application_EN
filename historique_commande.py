...
class HistoriqueCommandeScreen(Screen):
    ...
    # N°, Client, Date, Lieu, Total, Avance, Reste, Statut, VOIR, PAYER
    HEADERS    = ["N°", "Client", "Date/Heure", "Lieu", "Total (Ar)",
                  "Avance (Ar)", "Reste (Ar)", "Statut", "Voir", "Payer"]
    # Largeurs augmentées pour mobile
    COL_WIDTHS = [dp(45), dp(110), dp(125), dp(100), dp(95),
    ...
    def __init__(self, **kwargs):
        ...
        search_card = BoxLayout(
            ...
            size_hint=(1, None),
            height=dp(200),  # Augmenté pour nouveau champ
        ...
        col3 = BoxLayout(size_hint=(0.5, 1), spacing=dp(8))
        ...
        col3.add_widget(self.search_date)

        col4 = BoxLayout(size_hint=(0.5, 1), spacing=dp(8))
        col4.add_widget(Label(
            text="Lieu :",
            size_hint=(0.3, 1),
            color=TEXT_WHITE,
            font_size=sp(13),
        ...
        self.search_lieu = TextInput(
            ...
            size_hint=(0.7, 1),
            padding=[dp(8), dp(8)],
            hint_text="Lieu de paiement"
        )
        col4.add_widget(self.search_lieu)

        row2.add_widget(col3)
    ...
    def rechercher_commandes(self, instance):
        ...
        lieu = self.search_lieu.text.strip().lower()
        ...
        if lieu:
            # Récupérer le lieu de paiement (index 10 dans la commande)
            filtered_results = []
            for c in resultats:
                if len(c) > 10 and c[10]:
                    if lieu in str(c[10]).lower():
                        filtered_results.append(c)
                elif len(c) > 9 and c[9] and 'depot_sortie' not in str(c[9]):
                    if lieu in str(c[9]).lower():
                        filtered_results.append(c)
            resultats = filtered_results if filtered_results else []

        if date:
            filtered_results = []
            for c in resultats:
                if len(c) > 7 and c[7]:
                    date_commande = str(c[7])
                    if ' ' in date_commande:
    ...
    def actualiser_tableau(self):
        ...
        for idx, commande in enumerate(self.commandes):
            ...
            date_str = ""
            if len(commande) > 7 and commande[7]:
                d = str(commande[7])
                if ' ' in d:
                    parts = d.split(' ')
                    date_str = parts[0] + "\n" + parts[1][:5]
            ...
            # Récupérer le lieu de paiement (index 10)
            lieu_paiement = ""
            if len(commande) > 10 and commande[10]:
                lieu_paiement = str(commande[10])
            elif len(commande) > 9 and commande[9] and 'depot_sortie' not in str(commande[9]):
                # Fallback: utiliser depot_sortie si lieu_paiement est vide
                lieu_paiement = str(commande[9])
            else:
                lieu_paiement = "Non spécifié"

            # Tronquer le lieu si trop long
            if len(lieu_paiement) > 12:
                lieu_paiement = lieu_paiement[:10] + ".."

            valeurs = [
                str(commande[0]) if len(commande) > 0 else "",
                str(commande[1]) if len(commande) > 1 else "",
                date_str,
                lieu_paiement,  # Utiliser le lieu récupéré
            ...
            # 8 cellules de données
            for i, (val, w) in enumerate(zip(valeurs, self.COL_WIDTHS[:8])):
                tc = statut_color if i == 7 else None
                self.table.add_widget(
            ...
            # BOUTON VOIR (colonne 8)
            w_voir = self.COL_WIDTHS[8]
            btn_voir = Button(
                text="VOIR",
                size_hint=(None, None),
            ...
            # BOUTON PAYER (colonne 9)
            w_payer = self.COL_WIDTHS[9]
            btn_payer = Button(
                text="PAYER",
                size_hint=(None, None),
    ...
    def afficher_details_commande(self, commande_id):
        ...
        content = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(12))
        _bg(content, BG_DARK)

        # En-tête centré
        header = BoxLayout(orientation="vertical", size_hint=(1, None), height=dp(65), spacing=dp(5))
        _bg(header, BG_CARD, radius=dp(8))

        lbl_numero = Label(
            text=f"Commande N° [b]{commande_id}[/b]",
        ...
        depot_box = BoxLayout(size_hint=(1, None), height=dp(30), spacing=dp(10), padding=[dp(10), 0])
        _bg(depot_box, (0.15, 0.20, 0.35, 1), radius=dp(6))
        depot_box.add_widget(Label(
            text=f"DÉPÔT DE SORTIE : {depot_sortie}",
            font_size=sp(11), color=ACCENT_ORANGE,
            halign="center", valign="middle", bold=True
        ))
        content.add_widget(depot_box)

        # Lieu de paiement - CORRECTION
        lieu_paiement = commande.get('lieu_paiement', '')
        if not lieu_paiement:
            # Essayer avec depot_sortie si lieu_paiement est vide
            lieu_paiement = commande.get('depot_sortie', '')

        if not lieu_paiement or lieu_paiement == "":
            lieu_paiement = "Non spécifié"

        lieu_box = BoxLayout(size_hint=(1, None), height=dp(30), spacing=dp(10), padding=[dp(10), 0])
        _bg(lieu_box, (0.15, 0.20, 0.35, 1), radius=dp(6))
        lieu_box.add_widget(Label(
            text=f"LIEU DE PAIEMENT : {lieu_paiement}",
            font_size=sp(11), color=ACCENT_GREEN,
            halign="center", valign="middle", bold=True
        ))
        content.add_widget(lieu_box)
        ...
        for h in ["Produit", "Prix unit.", "Qte", "Total"]:
            lbl = Label(text=f"[b]{h}[/b]", markup=True,
                        font_size=sp(11), color=TEXT_WHITE,
        ...
        for i, p in enumerate(commande['produits']):
            ...
            for val in [
                ...
                lbl = Label(text=val, font_size=sp(11), color=TEXT_WHITE,
                            size_hint=(1, None), height=dp(38),
                            halign="center", valign="middle")
                lbl.bind(size=lambda inst, v: setattr(inst, "text_size", v))
                _bg(lbl, rc)
        ...
        nbre_produits = len(commande['produits'])
        hauteur_grid = max(nbre_produits + 1, 5) * 38
        scroll_g = ScrollView(
            size_hint=(1, None),
            height=min(hauteur_grid, dp(200)),
            do_scroll_y=True,
            do_scroll_x=False
        )
        ...
        # Boutons
        btn_box = BoxLayout(size_hint=(1, None), height=dp(48), spacing=dp(12), padding=[dp(10), dp(5)])
        btn_close = Button(
            text="FERMER",
    ...
    def generer_et_afficher_facture(self, commande_id, popup_to_close=None, mode_paiement=None, numero_cheque=""):
        ...
        try:
            ...
            depot_sortie = commande.get("depot_sortie", "Dépôt principal")
            lieu_paiement = commande.get("lieu_paiement", "Non spécifié")
    ...
    def ouvrir_popup_payer(self, commande_id, client, reste_actuel):
        # Récupérer la commande pour obtenir le dépôt de sortie et le lieu
        commande = get_commande_by_id(commande_id)
        depot_sortie_defaut = commande.get('depot_sortie', 'Dépôt principal') if commande else 'Dépôt principal'
        lieu_paiement_defaut = commande.get('lieu_paiement', '') if commande else ''
        ...
        # Dépôt de sortie (affiché mais pas modifiable - READ ONLY)
        ...
        # Lieu de paiement
        lieu_row = BoxLayout(size_hint=(1, None), height=dp(50), spacing=dp(10))
        ...
        self.lieu_paiement_input = TextInput(
            text=lieu_paiement_defaut,
            multiline=False,
            font_size=sp(14),
            foreground_color=TEXT_WHITE,
            background_color=(0.1, 0.16, 0.30, 1),
        ...
        lieu_row.add_widget(self.lieu_paiement_input)
        content.add_widget(lieu_row)

        # ========== LIGNE RENDU (cachée par défaut) ==========
        self.rendu_row = BoxLayout(size_hint=(1, None), height=0, spacing=dp(10))
        self.rendu_row.add_widget(Label(
            text="RENDU (Ar) :",
            size_hint=(0.3, 1),
            color=ACCENT_GREEN,
            font_size=sp(14),
        ...
        self.rendu_row.add_widget(self.rendu_input)
        content.add_widget(self.rendu_row)

        # Fonction pour calculer le rendu en temps réel
        def calculer_rendu_en_temps_reel(*args):
            try:
                montant_text = re.sub(r'[^0-9.]', '', montant_input.text or "0")
                montant = float(montant_text) if montant_text else 0
                mode = self.mode_paiement_spinner.text

                if mode == "Espèce" and montant > reste_actuel:
                    rendu = montant - reste_actuel
                    self.rendu_input.text = f"{rendu:,.0f}"
                    self.rendu_row.height = dp(50)
                    self.rendu_row.size_hint_y = None
                    self.rendu_row.opacity = 1
                else:
                    self.rendu_input.text = "0"
        ...
        # Lier l'événement de changement du montant et du mode
        montant_input.bind(text=calculer_rendu_en_temps_reel)
        ...
        def on_mode_change(spinner, text):
            if text == "Chèque":
                ...
                self.numero_cheque_input.disabled = False
                # Cacher le rendu pour le chèque
                self.rendu_row.height = 0
                self.rendu_row.size_hint_y = None
                self.rendu_row.opacity = 0
                self.rendu_input.text = "0"
            else:
                # Pour Espèce et Mvola, cacher le champ chèque
                self.cheque_row.height = 0
                self.cheque_row.size_hint_y = None
                self.cheque_row.opacity = 0
                self.numero_cheque_input.disabled = True
                self.numero_cheque_input.text = ""
                # Pour Espèce, activer le calcul du rendu
                if text == "Espèce":
                    calculer_rendu_en_temps_reel()
                else:
                    # Pour Mvola, cacher le rendu
                    self.rendu_row.height = 0
                    self.rendu_row.size_hint_y = None
                    self.rendu_row.opacity = 0
                    self.rendu_input.text = "0"
        ...
        popup = Popup(
            ...
            content=content,
            size_hint=(0.9, 0.9),  # Légèrement augmenté pour le rendu
            background_color=BG_CARD,
            title_size=sp(15)
        )
        ...
        btn_ok.bind(
            on_release=lambda inst: self.valider_paiement(
                ...
                self.depot_sortie_input.text, self.lieu_paiement_input.text,
                self.rendu_input.text  # Passer le rendu
            )
        )
        popup.open()

    def valider_paiement(self, popup, commande_id, client_nom, reste_actuel, montant_txt,
                     mode_paiement, numero_cheque, depot_sortie, lieu_paiement, rendu_txt="0"):
        ...
        try:
            ...
            # Récupérer la valeur du rendu
            rendu = float(re.sub(r'[^0-9.]', '', rendu_txt)) if rendu_txt else 0

            if montant <= 0:
                self.show_message("Erreur", "Montant invalide")
                return

            # Si c'est un paiement en espèce avec rendu, on ajuste le montant payé
            montant_a_enregistrer = montant
            if mode_paiement == "Espèce" and rendu > 0:
                # Le montant réellement encaissé est montant - rendu
                montant_a_enregistrer = montant - rendu

            if montant_a_enregistrer > reste_actuel:
                content = BoxLayout(orientation='vertical', spacing=dp(12), padding=dp(12))
                content.add_widget(Label(
                    text=f"Le montant saisi ({montant_a_enregistrer:,.0f} Ar) dépasse\nle reste à payer ({reste_actuel:,.0f} Ar).\n\nVoulez-vous continuer ?",
                    color=TEXT_WHITE,
                    font_size=sp(13)
                ))
                ...
                btn_oui.bind(on_release=lambda x: self._executer_paiement(
                    confirm_popup, popup, commande_id, client_nom, reste_actuel,
                    montant_a_enregistrer, mode_paiement, numero_cheque, depot_sortie, lieu_paiement, rendu
                ))
                btn_non.bind(on_release=confirm_popup.dismiss)
                confirm_popup.open()
            else:
                self._executer_paiement(
                    None, popup, commande_id, client_nom, reste_actuel,
                    montant_a_enregistrer, mode_paiement, numero_cheque, depot_sortie, lieu_paiement, rendu
                )
    ...
    def _executer_paiement(self, confirm_popup, paiement_popup, commande_id, client_nom,
                       reste_actuel, montant, mode_paiement, numero_cheque,
                       depot_sortie, lieu_paiement, rendu=0):
        ...
        try:
            ...
            # Mettre à jour le lieu de paiement si fourni
            if lieu_paiement:
                from database import update_commande_lieu_paiement
                update_commande_lieu_paiement(commande_id, lieu_paiement)
            ...
            if lieu_paiement:
                message += f"\nLieu de paiement : {lieu_paiement}"
            ...
            try:
                ...
                commande['lieu_paiement'] = lieu_paiement  # Mettre à jour le lieu dans l'objet
                ...
                filename = generer_facture_proforma(
                    ...
                    lieu_paiement=lieu_paiement,  # Passer le lieu à la facture
                    rendu=rendu
                )
