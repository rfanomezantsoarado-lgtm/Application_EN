...
class HistoriqueCommandeScreen(Screen):
    ...
    def actualiser_tableau(self):
        ...
        for idx, commande in enumerate(self.commandes):
            ...
            # 8 cellules de données (N°, Client, Date, Lieu, Total, Avance, Reste, Statut)
            for i, (val, w) in enumerate(zip(valeurs, self.COL_WIDTHS[:8])):
                tc = statut_color if i == 7 else None  # Statut à l'index 7
                self.table.add_widget(
    ...
    def afficher_details_commande(self, commande_id):
        ...
        # Dans afficher_details_commande, ajoutez ceci APRÈS avoir récupéré la commande :
        print("=== CLÉS DISPONIBLES ===")
        for key in commande.keys():
            print(f"Clé: '{key}' -> Valeur: {commande[key]}")
        # Lieu de paiement - CORRECTION
        # Lieu de paiement - Utiliser la même logique que le tableau
        ...
        # Afficher également pour déboguer
        print(f"Lieu trouvé: '{lieu_paiement}'")
        ...
        try:
            ...
            try:
                ...
                filename = generer_facture_proforma(
                    ...
                    lieu_paiement=lieu_paiement,
                    rendu=rendu  # Passer le rendu à la génération de la facture
                )
